...
export class MockPrismaService implements OnModuleInit, OnModuleDestroy {
  ...
  private findByConfirmationToken(token: string) {
    return Array.from(this.subscriptions.values()).find(sub => sub.confirmationToken === token) || null;
  }
  
  private findByUnsubscribeToken(token: string) {
    return Array.from(this.subscriptions.values()).find(sub => sub.unsubscribeToken === token) || null;
  }
} 